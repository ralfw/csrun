...
namespace csrun.tests
{
    ...
    public class Dissection_tests
    {
        ...
        public void Main_and_functions()
        {
            ...
            Assert.AreEqual(1, result[0].LineMappings.Mappings.First().DestinationFromLineNumber);
            Assert.AreEqual(2, result[0].LineMappings.Mappings.First().DestinationToLineNumber);
            
            Assert.AreEqual(Sourcecode.Sections.CSRunFunctions, result[1].Section);
            Assert.AreEqual(new[]{"f 1", "f 2", "f 3"}, result[1].Text);
            Assert.AreEqual(4, result[1].LineMappings.Mappings.First().DestinationFromLineNumber);
            Assert.AreEqual(6, result[1].LineMappings.Mappings.First().DestinationToLineNumber);
        }
        ...
        public void Functions_without_main()
        {
            ...
            Assert.AreEqual(2, result[0].LineMappings.Mappings.First().DestinationFromLineNumber);
            Assert.AreEqual(4, result[0].LineMappings.Mappings.First().DestinationToLineNumber);
        }
    }
}